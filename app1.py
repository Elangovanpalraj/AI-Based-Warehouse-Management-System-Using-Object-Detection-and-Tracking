...
def index():
    ...
    # Get latest 10 saved images
    image_folder = 'static/objects'
    image_files = sorted(os.listdir(image_folder), reverse=True)[:10]
    image_paths = [os.path.join(image_folder, img) for img in image_files]

    return render_template('index.html', items=items, images=image_paths)
